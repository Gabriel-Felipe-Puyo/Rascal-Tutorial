...
str printTaskWithDuration(ast) {
    rVal = [];
    // CORRECCIÓN 1: Agregamos 'prio' (p) a la estructura del for para poder usarla
...
str printTaskWithoutDuration(ast) {
    rVal = [];
    // CORRECCIÓN 2: Lo mismo aquí, pasamos la tupla <action, prio>
    for (<a, p> <- { <action, prio> | /task(action, prio, _) := ast }) {
